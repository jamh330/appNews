...
import { IonContent, IonInfiniteScroll, IonSlides, Platform } from '@ionic/angular';
...
export class Tab2Page implements OnInit{
  ...
  if(this.mode==0){
    ...
    this.newsService.getPanoramaCategories().subscribe(resp=>{      
      ...
      this.panoramaCategoriesId = this.panoramaCategories.map(object => object.id);
    }); 
  ...
  if(this.mode==0){
    ...
    this.currentIndex=   this.panoramaCategoriesId.indexOf(this.panoramaCategoriesId.find(select => select == this.selectedCategoryPanorama))
    
    

    if(this.categoryPlace.find(select => select == this.selectedCategoryPanorama)){
      await this.newsService.getLugarByCategories(this.page,this.selectedCategoryPanorama).subscribe(resp=>{
        console.log(resp);
        this.panoramas=resp.results;
        this.cargando = false;
    ...
    await this.newsService.getPanoramaByCategories(this.page,this.selectedCategoryPanorama).subscribe(resp=>{
      console.log(resp);
      this.panoramas=resp.results;
      this.cargando = false;
      if(resp.results.length >0){
...
async slideChanged(){
  ...
  this.selectedCategoryPanorama= this.panoramaCategories[this.currentIndex].id;
